...
namespace VrGrabber
{
    ...
    public class VrgGrabber : MonoBehaviour
    {
        ...
        {
            transform.localPosition = OVRInput.GetLocalControllerPosition(m_controller);//Device.instance.GetLocalPosition(side);
            transform.localRotation = OVRInput.GetLocalControllerRotation(m_controller);//Device.instance.GetLocalRotation(side);
        }

        void UpdateInput()
        {
            var preHoldInput = holdInput_;
            holdInput_ = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_controller);//OVRInput.Button.PrimaryHandTrigger; //Device.instance.GetHold(side);
        ...
        {
            if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, m_controller))//(OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger))//Device.instance.GetClick(side))
            {
        ...
        void FixedUpdateGrabbingObjectBySingleHand()
        {
            ...
            var stickY = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_controller);//Device.instance.GetCoord(side).y;
